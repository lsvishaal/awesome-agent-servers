...
async def test_handler_returns_response():
    ...
    with patch("competitor_analysis_agent.main._initialized", True), \
...
async def test_handler_with_multiple_messages():
    ...
    with patch("competitor_analysis_agent.main._initialized", True), \
         patch("competitor_analysis_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("competitor_analysis_agent.main._initialized", False), \
         patch("competitor_analysis_agent.main.initialize_all", new_callable=AsyncMock) as mock_init, \
         patch("competitor_analysis_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response), \
         patch("competitor_analysis_agent.main._init_lock"):
